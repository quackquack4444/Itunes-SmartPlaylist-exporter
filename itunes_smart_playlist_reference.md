...
| 0x39 | 57 | is_podcast_legacy | boolean | **not in the UI dropdown** — found only in Apple's own built-in "Recently Added"/"Recently Played"/"Top 25 Most Played" playlists, which all exclude podcasts via `NOT (is_podcast_legacy = TRUE)`. Inferred from context, not a UI label. |
...
Note the gap at 2^4 (16) — unconfirmed. Other kinds (TV Shows, Ringtones,
etc.) are almost certainly other powers of two.
...
## Unassigned field code gaps (0x39 confirmed real; rest still speculation)

The confirmed field codes above leave some conspicuous gaps: 0x01, 0x0F,
0x11, 0x13, 0x14, 0x15, 0x1A-0x1C, 0x1E, and most codes in 0x38-0x3B. None of
these have ever appeared in a criteria dump built through the Music.app UI —
but **0x39 is now confirmed to be a real, non-UI-exposed field**
(`is_podcast_legacy`, see the field table above), found only by inspecting
Apple's own built-in smart playlists from a fresh/empty library export
(Recently Added, Recently Played, Top 25 Most Played). Those built-ins can't
be recreated by hand through the criteria editor, so they're the only way to
surface fields the UI never lets you select. This is useful evidence that at
least some of the remaining gaps are real fields too, not simply unused
slots — though which ones, and what they represent, is still open.

For the rest, here's the same speculation as before, still unconfirmed:

- **0x13, 0x14, 0x15** sit as three *consecutive* gaps directly between
  Composer (0x12) and Plays (0x16). A plausible guess is **Work, Movement
  Name, and Movement Number** — real per-track metadata fields in Music.app
  (visible in Get Info) that, as far as we know, have never been exposed as
  filterable Smart Playlist criteria.
- **0x01** sits immediately after the nested-group sentinel (0x00) and before
  Name (0x02). No strong guess here — possibly an internal bookkeeping value
  (e.g. a row index) rather than a real filter field.
- **0x0F, 0x11, 0x1A-0x1C, 0x1E, 0x38, 0x3A, 0x3B**: isolated gaps with no
  obvious clustering — no confident guess.

If more built-in playlists surface more of these (e.g. a fresh library with
podcasts/audiobooks added might exercise more Distinguished-Kind-adjacent
logic), that remains the best way to find them — hand-built playlists
through the criteria UI structurally cannot reach fields the UI doesn't
expose.
...
- Full Media Kind bitmask beyond Music/Film/Podcast/Audiobook/Music
  Video/iTunes U (the 2^4=16 gap and TV Shows/Ringtones are the obvious
