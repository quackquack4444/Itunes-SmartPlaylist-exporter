...
CONFIRMED field codes (numeric ones needing the 68-byte block noted):
  ...
  category (string), 0x3C media_kind (numeric BITMASK enum -- see
  MEDIA_KIND_ENUM note below), 0x3E programme (string, distinct from
  Sort Programme/0x53), 0x3F series (numeric), 0x44 skips (numeric,
  skip count), 0x45 last_skipped (date), 0x47 album_artist (string),
  0x4E sort_name (string), 0x4F sort_album (string), 0x50 sort_artist
  (string), 0x51 sort_album_artist (string), 0x52 sort_composer
  (string), 0x53 sort_programme (string), 0x5A album_rating (numeric,
  same x20 scale as rating), 0x85 location (numeric enum, 1 = "on this
  computer"; other values e.g. iCloud unconfirmed), 0x86 icloud_status
  (numeric enum, 5 = "Local Only"; other values e.g. iCloud/Purchased/
  Matched/Uploaded unconfirmed).
...
Every field in the Music.app filter dropdown has now been reverse
  engineered at least once. Remaining unconfirmed detail is at the
  VALUE level for a few fields (see NOT YET UNDERSTOOD below and the
  enum notes above), not at the field-identity level.
...
# NOTE: values are powers of two (1=2^0, 2097152=2^21), so Media Kind is a
# BITMASK, not a sequential enum. Other kinds (Movies, TV Shows, Podcasts,
# Audiobooks, etc.) are almost certainly other powers of two, unconfirmed.
